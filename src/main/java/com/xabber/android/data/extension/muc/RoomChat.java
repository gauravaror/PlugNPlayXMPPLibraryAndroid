...
public class RoomChat extends AbstractChat {
     ...
     * Invited user for the sent packet ID.
     */
    private final ArrayList<String> invite_when_available;
    /**
    ...
    RoomChat(String account, String user, String nickname, String password, String subject) {
        ...
        invites = new HashMap<>();
        invite_when_available = new ArrayList<String>();
    }
    ...
    void setState(RoomState state) {
        ...
        if (state == RoomState.available) {
            sendMessages();
            for (String invitee:invite_when_available) {
                try {
                    MUCManager.getInstance().invite(account, getRoom(), invitee);
    ...
    void putInviteWhenAvailable( String user) {
        if (!invite_when_available.contains(user)) {
            invite_when_available.add(user);
        }
    }

    void removeInviteFromWhenAvailable( String user) {
        if (invite_when_available.contains(user)) {
            invite_when_available.remove(user);
        }
    }
    ...
    public ArrayList<String> getInvite_when_available() {
        return invite_when_available;
    }
}